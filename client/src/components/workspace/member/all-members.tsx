  ...
  const handleSelect = (roleId: string, memberId: string) => {
    ...
    mutate(payload, {
      ...
      onError: (error)=> {
        toast({
          title: "Error",
          description: "You cannot change roles",
          variant: "destructive",
        })
      }
    })
  ...
  return (
    <div className="grid gap-6 pt-2">
      ...
      {members?.map((member)=>{
        ...
        const initails = getAvatarFallbackText(name);
        const avatarColor = getAvatarColor(name);
        return (
        <div className="flex items-center justify-between space-x-4">
          <div className="flex items-center space-x-4">
            <Avatar className="h-8 w-8">
              <AvatarImage src={member.userId?.profilePicture || ""} alt="PFP" />
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  ...
                  disabled={!canChangeMemberRole || isLoading || member.userId._id === user?._id}
                >
